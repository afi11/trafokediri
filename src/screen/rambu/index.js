...
const urlPrintExcell = url + 'export/rambulalin/kpimsuwjw82jj8wjsuoidd';
const urlPrintPDF = url + 'export/rambulalin/sklksjsdsuwowow82k2';

const CountRambuByJalan = ({id}) => {
  const [count, setCount] = useState(0);

  useEffect(() => {
    axios
      .get(`${url}api/count_rambu_jalan/${id}`)
      .then((res) => {
        setCount(res.data);
      })
      .catch((err) => {});
  ...
  return <Text style={{marginTop: 5}}>Jumlah Rambu {count}</Text>;
};

const Rambu = ({navigation}) => {
  ...
  const gotoRambu = (id_jalan, nmjalan) => {
    navigation.navigate('ListRambu', {
      idJalan: id_jalan,
      namaJalan: nmjalan,
    });
  };
          ...
          <>
            ...
            <FlatList
              ...
              renderItem={({item}) => (
                <TouchableOpacity
                  onPress={() => {
                    gotoRambu(item.id_jalan, item.nama_jalan);
                  }}>
                  <View style={styles.listJalan.main}>
                    <View style={styles.listJalan.itemText}>
                      ...
                      <CountRambuByJalan id={item.id_jalan} />
                    </View>
