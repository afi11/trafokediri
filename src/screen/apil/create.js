...
const CreateApil = ({navigation}) => {
  ...
  const inputValidate = () => {
    ...
    if (apil.nama_foto === '') {
      fotoapil = 'Foto Apil Wajib diambil';
    }
    setEmpty({
      ...empty,
      ['id_jalan']: idjalan,
  ...
  const getJalan = () => {
    axios.get(`${url}api/jalan`).then((res) => {
      setJalan2(res.data);
    });
  };
  ...
  const postData = () => {
    setLoading(true);
    axios.post(`${url}api/create_apil`, apil).then((res) => {
      setLoading(false);
      Snackbar.show({
