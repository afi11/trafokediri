...
const urlPrintExcell = url + 'export/apil/dkkskwo2919192929292929kd211';
const urlPrintPDF = url + 'export/apil/sksjiiw929292922';

const CountApilByJalan = ({id}) => {
  const [count, setCount] = useState(0);

  useEffect(() => {
    axios
      .get(`${url}api/count_apil_byjalan/${id}`)
      .then((res) => {
        setCount(res.data);
      })
      .catch((err) => {});
  ...
  return <Text style={{marginTop: 5}}>Jumlah Apill {count}</Text>;
};

const Apil = ({navigation}) => {
  ...
  const gotoApil = (id_jalan, nmjalan) => {
    navigation.navigate('ListApil', {
      idJalan: id_jalan,
      namaJalan: nmjalan,
    });
  };
          ...
          <>
            ...
            <FlatList
              ...
              renderItem={({item}) => (
                <TouchableOpacity
                  onPress={() => {
                    gotoApil(item.id_jalan, item.nama_jalan);
                  }}>
                  <View style={styles.listJalan.main}>
                    <View style={styles.listJalan.itemText}>
                      ...
                      <CountApilByJalan id={item.id_jalan} />
                    </View>
